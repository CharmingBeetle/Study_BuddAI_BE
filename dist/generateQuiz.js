...
const createQuiz = (user_id, quiz_name, file_id) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    const questionsAmount = 4;
    const ai = new genai_1.GoogleGenAI({ apiKey: process.env.GOOGLE_API_KEY });
    // 1. get the file contents from the db
    const [rows] = yield connection_1.default.query(`SELECT * FROM files WHERE file_id = ? AND user_id = ?`, [
        file_id, user_id,
    ]);
